...
exports.userRegister = catchAsync(async (req, res, next) => {
  ...
  if (isExist && isExist.emailVerified == false) {
    ...
    await Notification.create({
      user: isExist?._id,
      content: `Hello ${isExist.fullName}. Congratulations for connect with us`,
    });
  ...
  await Notification.create({
    user: user?._id,
    content: `Hello ${user.fullName}. Congratulations for connect with us`,
  });
  return sendResponse(res, {
    statusCode: httpStatus.OK,
    success: true,
...
exports.allStatusData = catchAsync(async (req, res, next) => {
  ...
  const totalActiveSellerCount =
    totalresults.length > 0 ? totalresults[0].count : 0;
...
exports.sellerCount = catchAsync(async (req, res, next) => {
  if (req.user.role == "ADMIN" || "SUPER ADMIN") {
    ...
    sellersCount.forEach((item) => {
      ...
      const monthName = new Date(2000, item.month - 1)
...
exports.profileUpdate = catchAsync(async (req, res) => {
  ...
  if (user) {
    ...
    let userData = await User.findByIdAndUpdate(req.user._id, updateData, {
      new: true,
    });
...
exports.allAdminFetch = catchAsync(async (req, res) => {
  ...
  if (user.role == "ADMIN" || user.role == "SUPER ADMIN") {
    ...
    return sendResponse(res, {
      ...
      success: true,
      message:
        allAdmin.length > 0 ? "All admin fetch successfully" : "Not found",
      data: allAdmin,
    });
  } else {
    throw new ApiError(401, "You are unauthorized");
